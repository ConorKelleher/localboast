...
const RefComponent: React.FC<MemoizedRefComponentProps> = ({ refObj }) => {
  ...
  return <Text>Distinct ref objects:{refObjCountRef.current}</Text>
}
...
const UseUpdatingRefDemo = ({ value }: UseUpdatingRefDemoProps) => {
  ...
  return (
    <>
      ...
      <Text size="xl">
        ref.current === "{updatingRef.current}" (ref.current updated in effect,
        so won't have updated at render time)
      </Text>
